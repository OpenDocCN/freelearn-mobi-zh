...
当你已经知道面向对象编程（OOP）时，还需要使用协议和协议扩展吗？简短的答案是：不需要；然而，强烈推荐。在*第10章*，*协议导向设计*中，我们探讨了协议导向设计为何如此强大，以展示为什么你应该在协议导向编程（POP）中优先选择协议而不是面向对象编程（OOP）。通过理解协议和协议导向设计，你会更好地理解Swift标准库。
