...
本章中所有练习和活动的完整代码可在 GitHub 上找到，链接为[`packt.link/IIQmX`](https://packt.link/IIQmX)

# 依赖注入的必要性

我们一直使用`Application`类来创建这些组件的实例，并将它们传递给组件上层构造函数（我们创建了 API 和 Room 实例，然后是 Repository 实例，等等）。我们所做的是依赖注入的简化版本。
...
+   **Dagger**：这是一个为 Java 开发的依赖注入框架。它允许你将依赖分组到不同的模块中。你还可以定义组件，其中模块被添加以创建依赖图，Dagger 会自动实现以执行注入。它依赖于注解处理器来生成执行注入所需的代码。Dagger 的一个专门实现**Hilt**对 Android 应用程序非常有用，因为它消除了大量样板代码并简化了过程。
...
![图 13.1 – 练习 13.01 的模拟器输出，显示随机生成的数字](img/B19411_13_01.jpg)
...
如果你不想让 Dagger 管理`ClassB`的实例化，但仍然需要将`ClassA`的依赖注入，你可以使用字段注入，其形式可能如下所示：
...
1.  在`app/build.gradle`中添加 Hilt 插件：
...
此步骤的完整代码可以在[`packt.link/k7hs7`](https://packt.link/k7hs7)找到。
...
在前面的示例中，两个对象将有两个不同的生命周期。当使用**单例**符号提供依赖项时，整个应用程序生命周期中只会使用一个实例。这对于仓库、数据库和 API 组件很有用，因为多个实例对应用程序来说成本高昂。
...
![图 13.5 – 练习 13.04 的模拟器输出，显示随机生成的数字](img/B19411_13_05.jpg)
