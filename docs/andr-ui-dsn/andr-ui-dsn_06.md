...
![线框图搜索屏幕](img/B05061_6_8.jpg)

在这个线框图中，用户像往常一样进行搜索，但搜索结果屏幕包含三个新的按钮（**价格**、**时间**和**订阅...**），他们可以使用这些按钮根据不同的标准过滤搜索结果。
